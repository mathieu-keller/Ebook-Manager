    ...
    public BookDto getBookDto(String title) {
        ...
        return new BookDto(entity.getId(),
                ...
                Optional.ofNullable(entity.getCover())
                        .map(cover -> "data:image/jpg;base64," + new String(cover))
                        .orElse(null),
                Optional.ofNullable(entity.getContributorEntities())
                        .map(contributorEntities -> contributorEntities
                                .stream()
                                .map(contributorEntity -> new ContributorDto(contributorEntity.getId(), contributorEntity.getName()))
                                .toList()).orElse(null),
                null,
                null
        );
